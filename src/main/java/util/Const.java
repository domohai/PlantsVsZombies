package util;

public class Const {
	public static final int SCREEN_WIDTH = 820;
	public static final int SCREEN_HEIGHT = 637;
	public static final String SCREEN_TITLE = "Plants vs Zombies";
	
	public static final int ZOMBIE_HEIGHT = 142;
	public static final int ZOMBIE_WIDTH = 85;
	public static final int ZOMBIE_SPEED = -15;
	public static final int ZOMBIE_HP = 120;
	public static final int ZOMBIE_DAMAGE = 60;
	
	public static final int PLANT_WIDTH = 75;
	public static final int PLANT_HEIGHT = 90;
	public static final int PLANT_HP = 100;
	public static final int PLANT_DAMAGE = 50;
	public static final int BULLET_SPEED = 200;
	
	public static final int YARD_X = 60;
	public static final int YARD_WIDTH = 740;
	public static final int YARD_Y = 70;
	public static final int YARD_HEIGHT = 565;
	
	public static final int GRID_W = 80;
	public static final int GRID_H = 99;
	
	public static final float LINE_1 = 169.0f;
	public static final float LINE_2 = 268.0f;
	public static final float LINE_3 = 367.0f;
	public static final float LINE_4 = 466.0f;
	public static final float LINE_5 = 565.0f;
	
	public static final int BUTTON_WIDTH = 50;
	public static final int BUTTON_HEIGHT = 70;
	
	public static final int CONTAINER_OFFSET_Y = 115;
	public static final int TOOLBAR_TOP = 30;
	
	public static final int ZOMBIE_ZINDEX = 2;
	public static final int PLANT_ZINDEX = 0;
	public static final int BULLET_ZINDEX = 3;
	public static final int SUN_ZINDEX = 4;
	public static final int MOUSE_ZINDEX = 5;
	public static final int CONTAINER_ZINDEX = 1;
	
	public static final int GROUND_ZINDEX = -5;
	public static final int MIN_ZINDEX = -5;
	public static final int MAX_ZINDEX = 5;
	
	public static final float DEFAULT_FRAME_TIME = 0.16f;
	
}
